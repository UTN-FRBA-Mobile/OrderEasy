...
fun RequestTicket(navCont: NavController,userViewModel: UserViewModel,tableViewModel:TableViewModel) {
    Log.i("RequestTicket1-->",userViewModel.estadoUser.gastoTotDivide)
    Log.i("RequestTicket2-->",userViewModel.estadoUser.game.estado)
    Scaffold (
        topBar = { TopBar(userViewModel)},
        content = { innerPadding ->
            if(userViewModel.estadoUser.game.estado =="jugando"){
                navCont.navigate("game")
            }else if(userViewModel.estadoUser.game.estado!="nulo") {
                navCont.navigate("desafio")
            }else if(userViewModel.estadoUser.gastoTotDivide==null||userViewModel.estadoUser.gastoTotDivide=="") {
                LazyColumn(
                    modifier = Modifier
                        .fillMaxSize()
